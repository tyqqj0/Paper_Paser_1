...
class MetadataFetcher:
    ...
    async def fetch_metadata_waterfall(
        ...
        if not metadata and (identifiers.arxiv_id or identifiers.doi):
            ...
            try:
                ...
                metadata, raw_data = await self._fetch_from_semantic_scholar(identifier, id_type)
                if metadata:
    ...
    async def _fetch_from_crossref(self, doi: str) -> Tuple[Optional[MetadataModel], Dict]:
    ...
    async def _fetch_from_semantic_scholar(
        ...
        try:
            s2_data = await self.semantic_scholar_client.get_metadata(identifier, id_type)
            if not s2_data:
                return None, {}
    ...
    def _create_fallback_metadata(
        ...
        title = (
            ...
            source_data.get("url", "Unknown Title")
        )
