...
class ReferencesFetcher:
    ...
    async def fetch_references_waterfall(
        ...
        if identifiers.doi or identifiers.arxiv_id:
            ...
            try:
                references, raw_data = await self._fetch_from_semantic_scholar(
                    identifiers.doi or identifiers.arxiv_id,
                    "doi" if identifiers.doi else "arxiv"
                )
                if references:
                    logger.info(f"✅ Successfully fetched {len(references)} references from Semantic Scholar")
                else:
        ...
        if not references and pdf_content:
            ...
            try:
                references, raw_data = await self._fetch_from_grobid(pdf_content)
                if references:
                    logger.info(f"✅ Successfully extracted {len(references)} references from GROBID")
                else:
        ...
        if not references:
            logger.info("All references fetching methods failed, using empty references")
            references = []
            raw_data = {
                "source": "fallback",
                "message": "No references found through available methods",
                "attempted_sources": ["Semantic Scholar", "GROBID"] if pdf_content else ["Semantic Scholar"]
            }
    ...
    async def _fetch_from_semantic_scholar(
        ...
        try:
            ...
            references_data = await self.semantic_scholar_client.get_references(
                identifier, limit=50
            )
            ...
            for ref_data in references_data:
                ...
                authors = [author.get("name", "") for author in ref_data.get("authors", [])]
                ...
                parsed_data = {
                    ...
                    "year": year,
                    "venue": venue,
                    "doi": ref_data.get("externalIds", {}).get("DOI") if ref_data.get("externalIds") else None,
                    "arxiv": ref_data.get("externalIds", {}).get("ArXiv") if ref_data.get("externalIds") else None,
    ...
    async def _fetch_from_grobid(self, pdf_content: bytes) -> Tuple[List[ReferenceModel], Dict[str, Any]]:
        """
    ...
    def _create_fallback_references(self, source_data: Dict[str, Any]) -> List[ReferenceModel]:
        """
