...
async def connect_to_mongodb(
    ...
    NOTE: Function name kept as 'connect_to_mongodb' for compatibility,
    but actually connects to Neo4j.
...
async def disconnect_from_mongodb() -> None:
    ...
    NOTE: Function name kept for compatibility.
    """
    global _driver
...
def get_database() -> AsyncDriver:
    ...
    if _driver is None:
        raise RuntimeError("Neo4j driver not initialized. Call connect_to_mongodb() first.")
    
    return _driver


@asynccontextmanager
async def literature_collection() -> AsyncGenerator[AsyncSession, None]:
    """
    Get a Neo4j session context manager.
    
    NOTE: Function name kept for compatibility, but returns Neo4j session
    instead of MongoDB collection.
...
async def create_indexes() -> None:
    ...
    try:
        async with literature_collection() as session:
