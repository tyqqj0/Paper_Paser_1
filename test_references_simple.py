                        ...
                        if status_data.get("status") == "success":
                            ...
                            if literature_id:
                                ...
                                if lit_response.status_code == 200:
                                    ...
                                    print(f"\n=== 文献详情 ===")
                                    print(
                                        f"标题: {lit_data.get('metadata', {}).get('title', 'N/A')}"
                                    )
                                    ...
                                    if refs:
                                        ...
                                        if len(refs) > 5:
                                            print(
                                                f"  ... 还有 {len(refs) - 5} 个references"
                                            )
                                    else:
                                        print("❌ 没有找到references数据")
                                else:
                                    print(
                                        f"获取文献详情失败: {lit_response.status_code}"
                                    )
                                break
                        elif status_data.get("status") == "failure":
                            print(
                                f"❌ 任务失败: {status_data.get('error_message', 'Unknown error')}"
                            )
                            break
