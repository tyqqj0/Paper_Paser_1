            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "exists":
                    ...
                    if lit_response.status_code == 200:
                        ...
                        if refs:
                            ...
                            print(
                                f"解析率: {parsed_count}/{len(refs)} ({parsed_count/len(refs)*100:.1f}%)"
                            )
                        else:
                ...
                for i in range(30):  # 最多等待5分钟
                    ...
                    if status_response.status_code == 200:
                        ...
                        print(
                            f"任务状态: {status_data.get('status')} - {status_data.get('stage', 'N/A')}"
                        )
                        ...
                        if status_data.get("status") == "success":
                            ...
                            if lit_id:
                                ...
                                # 检查references
                                lit_response = await client.get(
                                    f"{BASE_URL}/literature/{lit_id}"
                                )
                                if lit_response.status_code == 200:
                                    lit_data = lit_response.json()
                                    refs = lit_data.get("references", [])
                                    ...
                                    if refs:
                                        ...
                                        for i, ref in enumerate(refs[:2], 1):
                                            print(
                                                f"  {i}. Source: {ref.get('source', 'N/A')}"
                                            )
                                            print(
                                                f"     Parsed: {bool(ref.get('parsed'))}"
                                            )
                                            if ref.get("parsed"):
                                                parsed = ref.get("parsed")
                                                print(
                            ...
                            print(
                                f"任务失败: {status_data.get('error_info', 'Unknown')}"
                            )
                            break
                else:
                    print("任务等待超时")
