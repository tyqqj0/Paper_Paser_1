...
class SystemTester:
    ...
    async def test_database_status(self) -> bool:
        ...
        try:
            ...
            if 'literature' in collections:
                ...
                if lit_count > 0:
                    # 展示最近文档
                    recent = await self.db.literature.find({}).sort("created_at", -1).limit(1).to_list(1)
                    if recent:
                        doc = recent[0]
                        print(f"📋 最新文档:")
    ...
    async def test_literature_processing(self, test_data: Dict[str, Any]) -> Dict[str, Any]:
        """测试文献处理流程"""
        print(f"\n📖 测试文献处理...")
    ...
    async def get_literature_details(self, literature_id: str) -> Dict[str, Any]:
        ...
        try:
            # 通过API获取
            response = await self.client.get(f"{API_BASE_URL}/api/literature/{literature_id}")
            if response.status_code == 200:
                api_data = response.json()
                print("✅ 通过API获取成功")
                
                # 直接从MongoDB获取完整数据
                mongo_data = await self.db.literature.find_one({"_id": ObjectId(literature_id)})
    ...
    def analyze_literature_data(self, api_data: Dict, mongo_data: Dict) -> Dict[str, Any]:
        """分析文献数据质量"""
        print("📊 分析数据质量...")
        ...
        # 基本信息分析
        analysis["basic_info"] = {
            "title_available": bool(api_data.get("title") and api_data["title"] != "Unknown Title"),
    ...
    def print_analysis_summary(self, analysis: Dict[str, Any]):
        ...
        print(f"   Semantic Scholar: {'✅' if metadata['semantic_scholar_data'] else '❌'}")
