...
from literature_parser_backend.db.mongodb import connect_to_mongodb, get_database
...
async def main():
    ...
    try:
            ...
            if count > 0:
                ...
                for i, doc in enumerate(recent_docs, 1):
                    ...
                    if content:
                        ...
                        print(
                            f"     文本长度: {len(content.get('full_text', '')) if content.get('full_text') else 0}",
                        )
            else:
                print("📭 暂无文献数据")
        else:
        ...
        if "tasks" in collections:
            ...
            if task_count > 0:
                recent_tasks = (
                    await db.tasks.find({}).sort("created_at", -1).limit(3).to_list(3)
                )
